...
class VramSchedulingEnv(gym.Env):
    ...
    def __init__(
        ...
        self.thermal_penalty_active: bool = False
    ...
    def reset(
        ...
        # --- NEW: Task handling logic ---
        task_id = options.get("task_id", "task_1_easy") if options else "task_1_easy"
        self.thermal_penalty_active = False
        
        if task_id == "task_1_easy":
            self.max_vram_mb = 8192.0 # 8GB Easy
        elif task_id == "task_2_medium":
            self.max_vram_mb = 6144.0 # 6GB Strict
        elif task_id == "task_3_hard":
            self.max_vram_mb = 6144.0
            self.thermal_penalty_active = True # Triggers heat constraints
        # ---------------------------------
    ...
    def step(self, action: int) -> tuple[dict[str, np.ndarray], float, bool, bool, dict]:
        ...
        if action == 0:
            if self.vram_usage_mb + memory_mb <= self.max_vram_mb:
                ...
                if self.thermal_penalty_active and self.vram_usage_mb > 0.8 * self.max_vram_mb:
                    reward -= 2.0
                    latency_ms += 5.0
            else:
                self.terminated = True
